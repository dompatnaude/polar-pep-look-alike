...
async function createOrder(req, res) {
  ...
  try {
    ...
    // 2. Confirm the cart contains items (join products for name + price).
    ...
    // 3. Calculate subtotal from item price * quantity.
    let subtotal = 0;
    ...
    // 4. Generate a unique order_number (retry on the rare UNIQUE collision).
    let order = null;
    ...
    // 6. Copy cart items into order_items (snapshot name + price).
    for (const it of itemsRes.rows) {
      await client.query(
        `INSERT INTO order_items (order_id, product_id, name, price, quantity)
         VALUES ($1,$2,$3,$4,$5)`,
        [order.id, it.product_id, it.name, money(it.price), it.quantity]
      );
    }
