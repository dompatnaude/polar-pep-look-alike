...
var DEFAULT_PRODUCT_IMAGE_URL = (window.PEPX_THEME_URI ? window.PEPX_THEME_URI + '/' : '') + 'assets/products/default-product.png';
...
var WP_FALLBACK_PRODUCTS = [
  {
    "id": "ghkcu_100_50",
    "name": "GHK-CU",
    "description": "Research-grade peptide reagent for controlled laboratory workflows.",
    "price": 100,
    "image": "",
    "inventory": 100,
    "category": "Repair",
    "tag": "REPAIR SUPPORT",
    "color": "#fdeef4;color:#db2777",
    "dosages": [
      "100mg",
      "50mg"
    ]
...
function loadProductsFromApi(){
  return fetch(apiUrl('/api/products'), { credentials: 'include' })
    ...
    .catch(function(){
      PRODUCTS = normalizeProductList(WP_FALLBACK_PRODUCTS);
      productsLoaded = true;
      return PRODUCTS;
    });
}
...
function startStripeCheckout(){
  ...
  return fetch(apiUrl('/api/create-checkout-session'), {
    ...
    // WordPress theme mode: cart still works; live Stripe needs Express API.
    showToast('Cart saved. Live Stripe checkout requires the Express backend.');
    if(checkoutBtn){
